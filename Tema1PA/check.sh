...
test_homework() {
	if [[ "$1" = "1" || "$1" = "bani" ]]; then
		test_bani
	elif [[ "$1" = "2" || "$2" = "gard" ]]; then
		test_gard
	elif [[ "$1" = "3" || "$3" = "bomboane" ]]; then
		test_bomboane
	elif [[ "$1" = "4" || "$4" = "sala" ]]; then
		test_sala
	elif [[ "$1" = "cs" ]]; then
...
test_bani() {
	n_tests=30
	src_names=(bani.c bani.cpp Bani.java)
...
test_gard() {
	n_tests=40
	src_names=(gard.c gard.cpp Gard.java)
	tests=( $(seq 0 $(($n_tests - 1))) )
	points=( $(printf '1.0 %.0s' $(seq 0 $(($n_tests - 1)))) )
	pmax=40
...
test_bomboane() {
	n_tests=45
	src_names=(bomboane.c bomboane.cpp Bomboane.java)
	tests=( $(seq 0 $(($n_tests - 1))) )
	points=( $(printf '1.0 %.0s' $(seq 0 $(($n_tests - 1)))) )
	pmax=45

	run_problem bomboane 3
}

# Task 4 - Bonus
test_sala() {
	n_tests=25
	src_names=(sala.c sala.cpp Sala.java)
	tests=( $(seq 0 $(($n_tests - 1))) )
	points=( $(printf '1.0 %.0s' $(seq 0 $(($n_tests - 1)))) )
	pmax=25
